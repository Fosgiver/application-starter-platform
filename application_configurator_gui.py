...
def create_review_lines(
    ...
    return (
        ...
        f"Application folder: {request.destination}",
        f"Access: {access_summary}",
        f"Email: {email_summary}",
        f"Database: {database_summary}",
        "",
       (
            "The installed application will be configured in place. "
...
def configure_installed_runtime_identity(
    request: DeveloperSetupRequest,
) -> None:
    technical_name = create_application_technical_name(
        request
    )
    database_filename = (
        create_application_database_filename(
...
def configure_installed_application(
    request: DeveloperSetupRequest,
) -> None:
    configure_installed_runtime_identity(request)
...
def start_installed_application(
    request: DeveloperSetupRequest,
) -> None:
    application_directory = request.destination
    ...
    run_script = application_directory / "run.py"
...
class DeveloperSetupWizard(tk.Tk):
    ...
    def _create_project_page(self) -> ttk.Frame:
        ...
        ttk.Label(
            page,
            text=(
              "Settings will be applied to the installed application. "
              "Existing configuration files will be overwritten."
            ),
            wraplength=650,
    ...
    def browse_destination(self) -> None:
       ...
       self.destination_var.set(str(application_directory))
    ...
    def _create_project_worker(
        ...
        try:
          configure_installed_application(request)
          start_installed_application(request)
    ...
    def poll_result(self) -> None:
        ...
        if succeeded:
            ...
            messagebox.showinfo(
                "Settings applied",
                "The installed application was configured "
                f"successfully:\n\n{result_text}",
                parent=self,
            )
            self.destroy()
